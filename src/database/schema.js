const fs = require("fs");
const path = require("path");

const SQL_DIR = path.join(__dirname, "../../sql");

async function tableExists(pool, tableName) {
  const result = await pool.query(
    "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = $1)",
    [tableName],
  );
  return result.rows[0].exists;
}

async function initSchema(pool) {
  const cdrExists = await tableExists(pool, "cdr");

  if (!cdrExists) {
    console.log("Fresh install — creating schema...");
    const createSql = fs.readFileSync(
      path.join(SQL_DIR, "CreateSchema.sql"),
      "utf8",
    );
    await pool.query(createSql);
    console.log("Schema created.");

    const populateSql = fs.readFileSync(
      path.join(SQL_DIR, "PopulateSchema.sql"),
      "utf8",
    );
    await pool.query(populateSql);
    console.log("Lookup tables populated.");
  } else {
    console.log("Existing database detected.");
  }

  const migrationSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration001_enrichment.sql"),
    "utf8",
  );
  await pool.query(migrationSql);

  const viewsSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration002_views.sql"),
    "utf8",
  );
  await pool.query(viewsSql);

  const starredSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration003_starred_calls.sql"),
    "utf8",
  );
  await pool.query(starredSql);

  const snapshotsSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration004_snapshots.sql"),
    "utf8",
  );
  await pool.query(snapshotsSql);

  const labelRulesSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration005_label_rules.sql"),
    "utf8",
  );
  await pool.query(labelRulesSql);

  const spamChecksSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration006_spam_checks.sql"),
    "utf8",
  );
  await pool.query(spamChecksSql);

  const labelExternalSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration007_label_external.sql"),
    "utf8",
  );
  await pool.query(labelExternalSql);

  const alertRulesSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration008_alert_rules.sql"),
    "utf8",
  );
  await pool.query(alertRulesSql);

  const alertRuleTypesSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration009_alert_rule_types.sql"),
    "utf8",
  );
  await pool.query(alertRuleTypesSql);

  const alertDirectionSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration010_alert_direction_and_scope.sql"),
    "utf8",
  );
  await pool.query(alertDirectionSql);

  const alertQualitySql = fs.readFileSync(
    path.join(SQL_DIR, "Migration011_alert_quality.sql"),
    "utf8",
  );
  await pool.query(alertQualitySql);

  const savedQueriesSql = fs.readFileSync(
    path.join(SQL_DIR, "Migration012_saved_queries.sql"),
    "utf8",
  );
  await pool.query(savedQueriesSql);
  console.log("Migrations applied.");
}

module.exports = { initSchema, tableExists };
